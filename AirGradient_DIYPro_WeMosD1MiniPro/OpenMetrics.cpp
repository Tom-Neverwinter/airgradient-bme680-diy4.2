...
/** Optional BME680 reading, defined in the main sketch (NAN when unavailable) */
...
String OpenMetrics::getPayload(void) {
  ...
  /** Optional BME680 add-on (not stock AG hardware, local-only) */
  if (bme680Found && !isnan(bme680Pressure)) {
    add_metric("pressure",
               "Barometric pressure as measured by the optional BME680 sensor, "
               "in hectopascals",
               "gauge", "hpa");
    add_metric_point("", String(bme680Pressure));
  }
