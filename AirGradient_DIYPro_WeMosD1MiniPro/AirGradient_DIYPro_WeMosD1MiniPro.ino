...
#define SHT_READ_MAX_RETRY 2          /** in-cycle retries before a read is failed */
...
/** Consecutive failed SHT read cycles, used to trigger bus/sensor recovery */
 ...
 * understood by the AirGradient cloud, so its readings are exposed only on the
 * local endpoints. These globals have external linkage and are read by
 * OpenMetrics.cpp and LocalServer.cpp via `extern`. NAN = no valid reading.
 */
/** Zanshin BME680 (talks to Wire directly). The Adafruit driver is avoided on
 *  purpose: it pulls in a second copy of Adafruit_BusIO, which collides with the
 *  copy the AirGradient library already vendors (multiple-definition link error). */
static BME680_Class bme680;
bool bme680Found = false;
float bme680Pressure = NAN;        /** hPa */
float bme680Temperature = NAN;     /** degC */
float bme680Humidity = NAN;        /** % */
float bme680GasResistance = NAN;   /** kOhm */
...
static void boardInit(void) {
  ...
  /** Optional BME680 add-on (pressure). Probed last so a missing/failed module
   *  never affects the stock sensors above. */
  bme680Init();
...
static void updateTvoc(void) {
  ...
  measurements.update(Measurements::TVOC, tvoc);
   ...
   *  getTvocIndex() is also invalid during the normal ~10s startup
   *  conditioning, so the threshold is deliberately generous to ignore that. */
  if (utils::isValidVOC(tvoc) || utils::isValidNOx(nox)) {
    sgpFailCount = 0;
    return;
  }

  if (sgpFailCount < 0xFF) {
    sgpFailCount++;
  }
...
/**
 * @brief Recover a stuck I2C bus, then re-init the bus.
 *
 * On the DIY Pro V4.2 the SHT3x shares the I2C bus with the SGP41 and OLED. A
 * glitch can leave a slave holding SDA low, which wedges every later transfer
 * (this is what makes the temperature "drop out" until a reboot). Per the I2C
 * spec, clocking SCL up to 9 times lets the slave finish its byte and release
 * SDA; we then issue a STOP and re-init Wire.
 */
static void i2cBusRecover(void) {
  int sda = ag.getI2cSdaPin();
  int scl = ag.getI2cSclPin();
...
/**
 * @brief Probe and configure the optional BME680 on the shared I2C bus.
 *
 * Tries both possible addresses (0x76 / 0x77). If nothing answers, bme680Found
 * stays false and the firmware runs exactly as the stock build does.
 */
static void bme680Init(void) {
  /** begin(speed) self-detects the BME680 at either 0x76 or 0x77. The lib calls
   *  Wire.begin(), which on the ESP8266 defaults to GPIO4/GPIO5 -- the same I2C
   *  pins this board already uses, so it does not disturb the other sensors. */
  if (bme680.begin(I2C_STANDARD_MODE)) {
    bme680Found = true;
    bme680.setOversampling(TemperatureSensor, Oversample8);
    bme680.setOversampling(HumiditySensor, Oversample2);
    bme680.setOversampling(PressureSensor, Oversample4);
    bme680.setIIRFilter(IIR4);
    bme680.setGas(320, 150); /** gas heater 320 degC for 150 ms */
    Serial.println("BME680 found (pressure on local /metrics + /measures)");
  } else {
    Serial.println("BME680 not found (optional sensor, skipping)");
  }
}

/**
 * @brief Read the BME680. Pressure is the headline value; temp/hum/gas are
 * captured too in case they are wanted later. On failure all values go NAN so
 * the local endpoints simply omit them.
 */
static void bme680Update(void) {
  if (!bme680Found) {
    return;
  ...
  bme680Humidity = hum / 1000.0f;         /** 1/1000 % -> % */
  bme680GasResistance = gas / 1000.0f;    /** milliohm -> ohm */

  Serial.printf("BME680: %.2f hPa, %.2f C, %.2f %%, %.0f ohm gas\r\n",
                bme680Pressure, bme680Temperature, bme680Humidity,
                bme680GasResistance);
}
...
static void tempHumUpdate(void) {
  ...
  if (shtFailCount < 0xFF) {
    shtFailCount++;
  }
  Serial.printf("SHT read failed (%u consecutive)\r\n", shtFailCount);

  /** Repeated failures mean the sensor or bus is wedged: clear the bus and
