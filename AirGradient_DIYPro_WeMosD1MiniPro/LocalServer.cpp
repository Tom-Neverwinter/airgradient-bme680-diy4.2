...
/** Optional BME680 readings, defined in the main sketch (NAN when unavailable) */
extern bool bme680Found;
extern float bme680Pressure;   /** hPa */
...
void LocalServer::_GET_measure(void) {
  ...
  /** Splice the optional BME680 pressure into the JSON object returned by the
   *  (untouched) library, just before its closing brace. */
  if (bme680Found && !isnan(bme680Pressure)) {
    int idx = toSend.lastIndexOf('}');
    if (idx > 0) {
      String sep = (toSend.charAt(idx - 1) == '{') ? "" : ",";
      toSend = toSend.substring(0, idx) + sep + "\"pressure\":" +
               String(bme680Pressure, 2) + toSend.substring(idx);
    }
  }
